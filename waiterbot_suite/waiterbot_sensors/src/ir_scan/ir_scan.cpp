...
namespace waiterbot
{
  ...
  IrScanNode::IrScanNode(ros::NodeHandle& n) : nh(n) { init(); }
  ...
  bool IrScanNode::spin()
  {
    ...
    while (ros::ok())
    {
      if(is_connected)
      {
        ...
        if (readRanges() == false)
        {
          ROS_WARN("IR Scan Node : Arduino disconnected");
          is_connected = false;
          continue;
        }
  ...
  void IrScanNode::computeRangesAndIntensity(unsigned int i, uint32_t reading)
  {
